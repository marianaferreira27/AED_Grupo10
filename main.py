...
def iniciar_sessao():
    ...
    def verificar_login():
        email = TxtEmail.get("1.0", "end").strip()  # Captura o texto do campo de e-mail
        password = TxtPalavraPasse.get("1.0", "end").strip()  # Captura o texto do campo de senha
        ...
        if email == "admin" and password == "admin":
            ...
            app.destroy()
            os.system("python admin.py")  # Abre a página específica para admins
        ...
        for linha in lista_users:
            ...
            if len(campos) >= 2 and campos[0] == email and campos[1] == password:
                CTkMessagebox.CTkMessagebox(title="Sucesso", message=f"Bem-vindo, {campos[0]}!", icon="check", option_1="Ok")
                app.destroy()
                os.system("python users_login.py")
